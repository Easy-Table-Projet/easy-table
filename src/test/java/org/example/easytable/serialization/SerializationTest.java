...
public class SerializationTest {
    private final SerializerUtil<ReservationCreateReqMessage> serializer = new SerializerUtil<>(ReservationCreateReqMessage.class);

    @Test
    public void testSerialize() {
        ReservationCreateReqMessage request = new ReservationCreateReqMessage(1L, 2L, null);
    ...
    public void testDeserialize() {
        ...
        ReservationCreateReqMessage request = serializer.deserialize(json);
    ...
    public void testSerializeAndDeserialize() throws JsonProcessingException {
        // 원본 객체 생성
        ReservationCreateReqMessage original = new ReservationCreateReqMessage(1L, 2L, null);
        // (만약 requestId가 내부에서 자동 생성된다면, 직렬화/역직렬화 과정에서 동일하게 유지되는지 확인합니다.)

        String json = serializer.serialize(original);
        ReservationCreateReqMessage deserialized = serializer.deserialize(json);
